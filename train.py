...
out_dir = os.environ.get("OUT_DIR", out_dir)  # override from env var
data_dir = os.environ.get("DATA_DIR", data_dir)  # override from env var
...
train_loader, val_loader = get_data_loaders(data_dir, batch_size, block_size, device)
...
def estimate_loss():
    ...
    model.eval()
    for split, loader in [
        ("train", make_iter(train_loader)),
        ("val", make_iter(val_loader)),
    ]:
        losses = torch.zeros(eval_iters)
        for k, (X, Y) in zip(range(eval_iters), iter(loader)):
            with ctx:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if wandb_log:
            wandb.log(
                ...
                )  # only log lr and mfu after the first eval
            )
        if losses["val"] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                if (
                    wandb_log and iter_num == eval_interval
